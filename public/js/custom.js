				...
				if (input.files) {
					...
					for(let i=0;i<input.files.length;++i){
						...
						if (ext === 'png'|| ext === 'jpg' || ext === 'jpeg') {
								...
								let $img=jQuery.parseHTML("<img class='preview-uploaded-portfolio' src=''>");
								filereader.onload = function(){
									$img[0].src=filereader.result;
								};
								filereader.readAsDataURL(input.files[i]);
								$(".portfolio-displayer").append($img);
							
						}else if(ext === 'mp4'|| ext === '3gp' || ext === 'mov' || ext == 'mkv'){
							//for video
							let filereader = new FileReader();
								let $img=jQuery.parseHTML('<video class="preview-uploaded-portfolio" controls><source src="" id="portfolio-video">Your browser does not support HTML5 video.</video>');
								filereader.onload = function(){
									$img[0].src=filereader.result;
								};
								filereader.readAsDataURL(input.files[i]);
